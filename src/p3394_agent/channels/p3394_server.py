...
class P3394ServerAdapter(ChannelAdapter):
    ...
    def authenticate_client(self, context: Dict[str, Any]) -> "ClientPrincipalAssertion":
        """
        Authenticate P3394 client with channel-dependent authentication.

        P3394 channel supports authentication based on:
        1. P3394 client address header (LOW assurance - self-asserted)
        2. Bearer token in Authorization header (MEDIUM assurance)
        3. Mutual TLS client certificate (HIGH assurance - future)

        Args:
            context: Request context with optional:
                - p3394_client_address: P3394 URI from x-p3394-client header
                - authorization: Bearer token from Authorization header
                - client_ip: Client IP address
                - websocket_session_id: WebSocket session ID
        ...
        # Extract context
        p3394_address = context.get("p3394_client_address")
        authorization = context.get("authorization")
        client_ip = context.get("client_ip", "unknown")
        ws_session_id = context.get("websocket_session_id")

        # 1. Check for Bearer token (MEDIUM assurance)
        if authorization and authorization.startswith("Bearer "):
            token = authorization[7:]
            # In a full implementation, validate the token
            channel_identity = f"token:{token[:8]}..."
            ...
            return self.create_client_assertion(
                ...
                metadata={
                    "client_ip": client_ip,
                    "p3394_address": p3394_address,
                    "note": "Authenticated via Bearer token"
                }
            )

        # 2. P3394 client address provided (LOW assurance - self-asserted)
        if p3394_address:
            channel_identity = f"p3394:{p3394_address}"

            return self.create_client_assertion(
                channel_identity=channel_identity,
                assurance_level=AssuranceLevel.LOW,
                authentication_method="p3394_self_asserted",
                metadata={
                    "client_ip": client_ip,
                    "p3394_address": p3394_address,
                    "websocket_session": ws_session_id,
                    "note": "Self-asserted P3394 agent identity"
                }
            )

        # 3. Fall back to anonymous (NONE assurance)
        return self.create_client_assertion(
            ...
            metadata={
                "client_ip": client_ip,
                "note": "Anonymous P3394 client. Provide x-p3394-client header for identity."
            }
        )

...
class CLIChannelAdapter(ChannelAdapter):
    ...
    def authenticate_client(self, context: Dict[str, Any]) -> "ClientPrincipalAssertion":
        """
        Authenticate CLI client with channel-dependent capability authentication.

        REQ-CAP-CH-AUTH Implementation:

        CLI channel supports IMPLICIT authentication based on:
        1. OS user identity (LOW assurance)
        2. SSH key fingerprint (MEDIUM assurance)
        3. Local socket ownership (MEDIUM assurance)

        Authentication levels:
        - NONE: Anonymous user (no OS identity detected)
        - LOW: OS user identity verified
        - MEDIUM: SSH key or verified local process
        - HIGH: Explicit credential elevation via /login

        Credential elevation:
        When accessing capabilities requiring higher assurance/authorization,
        the gateway will challenge for explicit authentication via /login.

        Args:
            context: Connection context with optional:
                - username: OS username
                - ssh_key_fingerprint: SSH key fingerprint
                - peername: Socket peer info
                - session: Existing session (for elevation)
        ...
        ssh_key_fp = context.get("ssh_key_fingerprint")
        peername = context.get("peername")
        session = context.get("session")

        # Detect OS user if not provided
        if not username:
            try:
        ...
        # Check if this is credential elevation (session already exists with principal)
        if session and session.client_principal_id and session.client_principal_id != "urn:principal:org:public:role:anonymous:person:guest":
            # Already authenticated - return existing principal assertion
            return self.create_client_assertion(
                channel_identity=session.client_principal_id,
                assurance_level=AssuranceLevel(session.assurance_level),
                authentication_method="session_continuation",
                metadata={
        ...
        # 1. Check for SSH key authentication (MEDIUM assurance)
        if ssh_key_fp:
            # SSH key provides MEDIUM assurance
            channel_identity = f"ssh:{username}:{ssh_key_fp}"
            ...
            return self.create_client_assertion(
                ...
                metadata={
                    ...
                    "pid": os.getpid(),
                    "note": "Authenticated via SSH key"
                }
            )

        # 2. Check for local socket connection (MEDIUM assurance if same user)
        if peername and self._is_local_connection(peername):
            # Local process connection provides MEDIUM assurance
            channel_identity = f"local:{username}:{os.getpid()}"

            return self.create_client_assertion(
                channel_identity=channel_identity,
                assurance_level=AssuranceLevel.MEDIUM,
                authentication_method="local_socket",
                metadata={
                    "os_user": username,
                    "pid": os.getpid(),
                    "socket_peer": str(peername),
                    "note": "Authenticated via local socket ownership"
                }
            )

        # 3. OS user identity only (LOW assurance)
        if username and username != "unknown":
            channel_identity = f"os_user:{username}"

            return self.create_client_assertion(
                channel_identity=channel_identity,
                assurance_level=AssuranceLevel.LOW,
                authentication_method="os_user",
                metadata={
                    "os_user": username,
                    "pid": os.getpid(),
                    "note": "Authenticated via OS user identity. Use /login for higher privileges."
                }
            )

        # 4. Fall back to anonymous (NONE assurance)
        return self.create_client_assertion(
            channel_identity="anonymous",
            assurance_level=AssuranceLevel.NONE,
            authentication_method="unauthenticated",
            metadata={
                "os_user": username,
                "pid": os.getpid(),
                "cwd": os.getcwd(),
                "note": "Anonymous user. Use /login <api_key> to authenticate."
            }
        )

    def _is_local_connection(self, peername) -> bool:
        """Check if connection is from local machine (Unix socket or localhost)"""
        if not peername:
            return False
        ...
        # TCP localhost connections
        if isinstance(peername, tuple):
            host = peername[0] if len(peername) > 0 else None
    ...
    async def handle_cli_client(
        ...
        # Create session for this client with connection context
        session = await self.gateway.session_manager.create_session(channel_id="cli")
        session_id = session.id
        self.clients[session_id] = writer

        # Store connection context in session metadata
        session.metadata["connection_context"] = {
    ...
    def _cli_to_umf(self, cli_message: dict, session_id: str) -> P3394Message:
        ...
        if session and "connection_context" in session.metadata:
            conn_ctx = session.metadata["connection_context"]
            auth_context.update({
                "peername": conn_ctx.get("peername"),
                "sockname": conn_ctx.get("sockname"),
                "connection_type": conn_ctx.get("connection_type")
            })

        # Authenticate client with full context
        client_assertion = self.authenticate_client(auth_context)

        # Create UMF message with embedded client assertion
        return P3394Message(
            ...
            metadata={
                ...
                "channel": {
                    ...
                    "connection_context": auth_context
                }
            }
        )
